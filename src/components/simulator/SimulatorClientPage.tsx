...
// Este componente recebe os parâmetros da URL como props
export function SimulatorClientPage({ params }: { params: { step: string } }) {
  const { goToStep } = useSimulatorStore((state) => state.actions);
  const currentStepFromURL = parseInt(params.step, 10);

  // Efeito para sincronizar a URL com o estado do Zustand
  useEffect(() => {
    if (!isNaN(currentStepFromURL)) {
